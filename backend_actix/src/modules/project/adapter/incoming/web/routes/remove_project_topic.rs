...
pub async fn remove_project_topic_handler(
    user: VerifiedUser,
    path: web::Path<Uuid>,
    body: web::Json<RemoveProjectTopicRequest>,
...
mod tests {
    ...
    /* --------------------------------------------------
     * Mock RemoveProjectTopicUseCase
     * -------------------------------------------------- */

    #[derive(Clone)]
    struct MockRemoveProjectTopicUseCase {
        result: Result<(), RemoveProjectTopicError>,
    }

    impl MockRemoveProjectTopicUseCase {
        fn success() -> Self {
            Self { result: Ok(()) }
        }

        fn error(err: RemoveProjectTopicError) -> Self {
            Self { result: Err(err) }
        }
    }

    #[async_trait]
    impl RemoveProjectTopicUseCase for MockRemoveProjectTopicUseCase {
        async fn execute(
            &self,
            _owner: UserId,
            _project_id: Uuid,
    ...
    async fn test_remove_project_topic_success() {
        ...
        let app_state = TestAppStateBuilder::default()
            .with_remove_project_topic(MockRemoveProjectTopicUseCase::success())
            .build();
        ...
        let app = test::init_service(
            App::new()
                ...
                .service(remove_project_topic_handler),
        )
        .await;
    ...
    async fn test_remove_project_topic_project_not_found() {
        ...
        let app_state = TestAppStateBuilder::default()
            .with_remove_project_topic(MockRemoveProjectTopicUseCase::error(
    ...
    #[actix_web::test]
    async fn test_remove_project_topic_topic_not_found() {
        let user_id = Uuid::new_v4();
        let project_id = Uuid::new_v4();
        let topic_id = Uuid::new_v4();
    ...
    #[actix_web::test]
    async fn test_remove_project_topic_repository_error_internal_error() {
        let user_id = Uuid::new_v4();
        let project_id = Uuid::new_v4();
        let topic_id = Uuid::new_v4();
        ...
        let app_state = TestAppStateBuilder::default()
            .with_remove_project_topic(MockRemoveProjectTopicUseCase::error(
                RemoveProjectTopicError::RepositoryError("db down".to_string()),
            ))
            .build();
    ...
    #[actix_web::test]
    async fn test_remove_project_topic_unverified_user_forbidden() {
        let user_id = Uuid::new_v4();
        let project_id = Uuid::new_v4();
        let topic_id = Uuid::new_v4();
