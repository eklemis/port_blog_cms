...
///
/// Removes the post outright, unlike the archive on `DELETE /api/blog/{post_id}`. Topic links go with it by cascade. Not reversible.
#[utoipa::path(
    delete,
    path = "/api/blog/{post_id}/hard",
    tag = "blog",
...
#[delete("/api/blog/{post_id}/hard")]
pub async fn hard_delete_blog_post_handler(
    user: VerifiedUser,
    path: web::Path<Uuid>,
...
mod tests {
    ...
    use crate::blog::application::ports::incoming::use_cases::HardDeleteBlogPostUseCase;
    ...
    #[async_trait]
    impl HardDeleteBlogPostUseCase for Mock {
            ...
            App::new()
                .app_data(
                    TestAppStateBuilder::default()
                        .with_blog_hard_delete(Mock { result })
                        .build(),
                )
                .app_data(web::Data::new(provider))
                .service(hard_delete_blog_post_handler),
        )
        .await;

        let req = test::TestRequest::delete()
            .uri(&format!("/api/blog/{}/hard", Uuid::new_v4()))
