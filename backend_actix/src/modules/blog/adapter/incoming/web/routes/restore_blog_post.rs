...
///
/// Brings back a post archived with `DELETE /api/blog/{post_id}`. Its publication state is untouched, so an archived draft returns as a draft.
#[utoipa::path(
    post,
    path = "/api/blog/{post_id}/restore",
    tag = "blog",
    params(("post_id" = Uuid, Path, description = "Identifier of the post")),
    responses(
        (status = 204, description = "Post restored"),
...
#[post("/api/blog/{post_id}/restore")]
pub async fn restore_blog_post_handler(
    user: VerifiedUser,
    path: web::Path<Uuid>,
...
mod tests {
    ...
    use crate::blog::application::ports::incoming::use_cases::RestoreBlogPostUseCase;
    ...
    #[async_trait]
    impl RestoreBlogPostUseCase for Mock {
            ...
            App::new()
                .app_data(
                    TestAppStateBuilder::default()
                        .with_blog_restore(Mock { result })
                        .build(),
                )
                .app_data(web::Data::new(provider))
                .service(restore_blog_post_handler),
        )
        .await;

        let req = test::TestRequest::post()
            .uri(&format!("/api/blog/{}/restore", Uuid::new_v4()))
